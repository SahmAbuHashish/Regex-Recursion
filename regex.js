...
function capitalA(s){
    // Add your logic.
        ...
        if(rejx3.test(s) || rejx4.test(s)){
            if(rejx.test(s) && rejx2.test(s)){
                return vlaue = true;
            }else{
                return vlaue = false
            }
...
function ioEmail(email){
    ...
    if(rej.test(email) && rej1.test(email)){
        val = true
    }else{
        val = false
    }
...
function imagesSearcher(text){
    let arr = [];
    // Add your logic.
    let rej1 = /\.(jpg|jpeg|png)/gi;
    let rej =  /\.(jpg|jpeg|png)$/gi;

    if(rej1.test(text)){
        arr.push(text.match(rej1))
    }
    // return text.match(rej1)
    return arr
}
